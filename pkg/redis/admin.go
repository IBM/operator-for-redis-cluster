...
func (a *Admin) GetKeysInSlot(addr string, slot Slot, batch int, limit bool) ([]string, error) {
	...
	for {
		var resp resp2.Any
		cmdErr := c.DoCmd(&resp,"CLUSTER GETKEYSINSLOT", slot.String(), strconv.Itoa(batch))
		if err := a.Connections().ValidateResp(&resp, cmdErr, addr, "Unable to run command GETKEYSINSLOT"); err != nil {
			return allKeys, err
		}
		if err != nil {
...
func (a *Admin) MigrateKeys(addr string, dest *Node, slots SlotSlice, batch int, timeout int, replace bool) (int, error) {
	...
	for _, slot := range slots {
		for {
			var resp resp2.Any
			cmdErr := c.DoCmd(&resp, "CLUSTER GETKEYSINSLOT", slot.String(), batchStr)
			if err := a.Connections().ValidateResp(&resp, cmdErr, addr, "Unable to run command GETKEYSINSLOT"); err != nil {
				return keyCount, err
			}
			keys := resp.I.([]string)
